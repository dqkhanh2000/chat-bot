...
def get_model():
  ...
  if classification_model is None:
    data_his = read_data_file("vec.txt")
    process_classification_data(print_log=False)
    data_new = read_data_file("vec.txt")
    has_change = False
...
def train():
  data = np.array(read_data_file("vec.txt"))
  data = np.reshape(data, (data.shape[0], 1, data.shape[1]))
  label = np.array(read_data_file("label.txt"))
  num_label = len(np.unique(label))
  
  x_train, x_test, y_train, y_test = train_test_split(data, label, test_size=0.3, random_state=42)
  
  # w2v_model = load_word2vec_model("model/model.bin")
  # vocabs_len = len(w2v_model.get_words())
  # w2v_dim = w2v_model.get_dimension()
  ...
  model = Sequential()
  # model.add(Embedding(VOCABULARY_LENGTH, WORD2VEC_DIM, input_length=WORD2VEC_DIM))
  ...
  model.summary()
  model.fit(
    x_train, y_train, validation_data=(x_test, y_test), batch_size=10, epochs=100
  )
